...
public record SerializerInfo(
        String fieldName,
        String label,
        List<IFileFieldDefinition> defaultFields,
    ...
    public static final Codec<SerializerInfo> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            IFileFieldDefinition.LIST_CODEC.optionalFieldOf("defaultFields", Collections.emptyList()).forGetter(SerializerInfo::defaultFields),
            ResourceId.CODEC.optionalFieldOf("defaultType").forGetter(SerializerInfo::defaultType)
    ).apply(instance, SerializerInfo::new));

    public static SerializerInfo of(String fieldName, String label, IFileFieldDefinition... defaultFields) {
