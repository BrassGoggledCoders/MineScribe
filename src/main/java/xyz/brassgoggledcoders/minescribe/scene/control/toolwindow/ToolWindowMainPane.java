...
public class ToolWindowMainPane extends BorderPane {
    ...
    private void handleWindowUpdate(ToolWindowLocation toolWindowLocation, @Nullable ToolWindow toolWindow) {
        Node node = null;
        if (toolWindow != null) {
            node = new ToolWindowPane(toolWindow);
        }
        if (node != null) {
            node.getProperties()
